...
//
class ScoreRecord {
    constructor(name, score, isCurrentPlayer) {
        this.name = name;
        this.score = score;
...
function getHighScoreTable() {
    ...
    for (var i = 0; i < NUMBER_OF_HIGH_SCORES; i++) {
        ...
        table.push(new ScoreRecord(record[0], record[1], false));
    }
...
function addHighScore(record, node) {
    ...
    if (record.isCurrentPlayer) {
        name.style.fill = "blue";
        score.style.fill = "blue";
    }
